...
gapminder  %>%
  ...
  ggplot(aes(x = gdpPercap, y = lifeExp)) +
  geom_point() + 
  geom_smooth(method = "lm", formula = y ~ x, se = T) +      # straight line
  geom_smooth(method = "lm", formula = y ~ poly(x, 2), se = T)  # polynomial line
...
# We can ask R to compute the difference in R-square between the linear and 
#  quadratic model. 
# The data objects "fit_..." contain the r-square as a variable: "r.square" 
# To get the difference we just subtract them:

summary(fit_quadratic)$r.squared - summary(fit_linear_1000)$r.squared

#  = 0.0912, i.e. about 9%. 
#
#  The quadratic model reduces the size of the residuals by 9% MORE than
#   the linear model. This is modest evidence that the relationship between
#   gdp and life expectancy is weaker at high levels of gdp compared to lower
#   levels, i.e. "diminishing returns". 
...
#  1. according to the linear model, what is the "predicted" mean life-expectancy
#      for an Asian country with a GDP per capita of $20,000 in 2002?
...
# Answers
#
# 1. The model is y_hat = b_0 + x*b_1
summary(fit_linear_gdp1000)
# b_0 is 63.78378
# b_1 is 0.53568
# x is 20

q1_lin <- 63.78378 + (20*0.53568)
q1_lin

# answer is 74.50


# 2. The model is y_hat = b_0 + x*b_1 + x^2*b_2
summary(fit_quadratic)
# b_0 is 61.66104
# b_1 is 1.21256
# b_2 is -0.02126
...
q1_quad <- 61.66104 + (20*1.21256) + (400*-0.02126)
q1_quad               

# answer is 77.41 years


#Notes:
#(R2) tell us that how much of the variablity in the data would be accounted 
#for by knowing the x variable 
